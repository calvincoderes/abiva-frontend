...
import { Spin, Row, Col, Input, Pagination } from 'antd'
  ...
  <ViewComponent>
    {({ state, data, pagination, handlesearchTable, handleOnChange }) => (
      <Styled>
        <Row gutter={[20, 20]}>
          <Col span={24}>
            <p style={{ fontSize: '24px', fontWeight: 'bold', color: 'var(--tory-blue)' }}>MODULES</p>
          </Col>
          <Col span={10}>
            <Input onPressEnter={handlesearchTable} size='large' placeholder='Search' prefix={<SearchOutlined />} />
          </Col>
          <Col span={24} className='mt-3'>
            <Spin spinning={state.studentM.status === 1}>
              <Row gutter={[30, 30]}>
                {data && data.map((v, k) => (<Col key={k} span={8}>
                  <AssessmentBox data={v} url={`/modules/${v.id}`} />
                </Col>))}
