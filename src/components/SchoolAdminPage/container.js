...
import { USER_TYPE_SCHOOL_ADMIN } from '@/utils/constants'
...
const Container = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (status === 2 && payload && schools) {
      ...
      if (Object.keys(payload).length > 0) actions.setUsers({ status: 0, payload: null })
    } else if (status === 3) {
  ...
  useEffect(() => {
    ...
    if (status === 2) {
      actions.getUsers({ type: USER_TYPE_SCHOOL_ADMIN })
