...
const Container = ({ children, mainFormik }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit (values) {
      if (values && isFormSubmitted === false) {
        values.key = (mainFormik.values.books_data ? mainFormik.values.books_data.length : 0)
  ...
  const handleSubmit = () => {
    if (data && Object.keys(data).length > 0) {
      ...
      actions.updateSchool(mainFormik.values.school_data)
    }
  }
  ...
  useEffect(() => {
    if (mainFormik.values.books_data && Object.keys(mainFormik.values.books_data).length > 0 && isListsLoaded === false && allCategories && allSubCategories && allContent) {
      handlePushedData(filteredSchoolData(mainFormik.values.books_data)) // Reference for Deleting per row
  ...
  useEffect(() => {
    const { status } = state.updatedSchool
    if (status === 2) {
      actions.setUpdatedSchool(clearState)
      history.push('/school')
    } else if (status === 3) {
      actions.setUpdatedSchool(clearState)
    }
