...
const View = ({ mainFormik }) => (
  <ViewComponent mainFormik={mainFormik}>
    {({
      ...
      <Styled>
        <Spin spinning={state.updatedSchool.status === 1 || state.content.status === 1 || state.categories.status === 1 || state.subCategories.status === 1}>
          <Row gutter={[16, 16]}>
            <Col span={24}>
          ...
          <Row className='mt-5'>
            ...
            <Col span={6}>
              <Button
                ...
                disabled={state.updatedSchool.status === 1 || Object.keys(data).length === 0}
                block
                type='primary'
                size='large'
                style={{ borderRadius: '4px' }}
